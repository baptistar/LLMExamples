...
for _ in range(5):
    ...
    # run sampler
    print('======== Deterministic Sampling (No skill) =========')
    QW.generate([task], stochastic=False)
    for _ in range(num_random):
        print('======== Stochastic Sampling (No skill) =========')
        QW.generate([task], stochastic=True)
        print('\n')
    # run sampler
    print('======== Deterministic Sampling (+ skill) =========')
    QW.generate([prompting_skills, task], stochastic=False)
    for _ in range(num_random):
